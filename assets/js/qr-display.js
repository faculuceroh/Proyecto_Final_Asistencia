...
(function () {
  ...
  const tokenUrl      = wrap.dataset.tokenUrl      || '';
  const scanBase      = wrap.dataset.scanBase      || '../alumno/escanear.php';
  const presentesUrl  = wrap.dataset.presentesUrl  || '';
  const finalizarUrl  = wrap.dataset.finalizarUrl  || '';
  ...
  /* ---------- Polling de presentes ---------- */
  function renderLiveList(ultimos) {
    if (!liveList) return;
    liveList.innerHTML = '';
    ultimos.forEach(function (u) {
      const item = document.createElement('div');
      item.className = 'live-item';
      item.innerHTML = `<div class="mini-avatar">${u.iniciales}</div>
        <span>${u.nombre}</span><span class="t">${u.hora}</span>`;
  ...
  async function pollPresence() {
    if (!presentesUrl) {
      ...
      setInterval(() => {
        if (i < demoNames.length && presentes < totalAlum) {
          updatePresenceCount(presentes + 1);
          if (liveList) {
            const item = document.createElement('div');
            item.className = 'live-item';
            item.innerHTML = `<div class="mini-avatar">${demoNames[i][1]}</div>
              <span>${demoNames[i][0]}</span><span class="t">${App.formatTime()}</span>`;
    ...
    // Producción: polling real cada 5 segundos
    async function fetchPresentes() {
      try {
        const data = await App.api(presentesUrl);
        updatePresenceCount(data.presentes);
        renderLiveList(data.ultimos || []);
      } catch (_) { /* silencioso — no interrumpe el QR */ }
    }
    fetchPresentes();
    setInterval(fetchPresentes, 5000);
  }
