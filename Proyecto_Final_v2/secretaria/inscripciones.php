...
// Todos los alumnos para el select de alta manual
$todos_alumnos = $pdo->query(
    "SELECT id, legajo, CONCAT(apellido,', ',nombre) AS nombre_completo
     FROM usuarios WHERE rol='alumno' AND activo=1 ORDER BY apellido"
)->fetchAll();
                ...
                <select class="select" name="alumno_id" id="alumnoSelect" required>
                  ...
                  <?php foreach ($todos_alumnos as $a): ?>
                    <option value="<?= $a['id'] ?>" data-legajo="<?= $a['legajo'] ?>">
                      <?= htmlspecialchars($a['nombre_completo']) ?> · <?= $a['legajo'] ?>
                    </option>
                  <?php endforeach; ?>
                </select>
              </div>
