...
$stmt = $pdo->prepare(
    'SELECT c.id, c.estado, c.hora_inicio, c.duracion_min,
     ...
     WHERE c.id = ? AND (m.profesor_id = ? OR m.profesor_2_id = ?) AND c.fecha = CURDATE()
     LIMIT 1'
);
...
// Aulas disponibles
$aulas = $pdo->query('SELECT id, nombre, token FROM aulas WHERE activo = 1 ORDER BY nombre')->fetchAll();
